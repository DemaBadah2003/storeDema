...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    // الكرت مُعدل ليصبح ثلاثي الأبعاد بخلفية زجاجية بلورية ناعمة ومتناسقة
    <div className="bg-white/70 backdrop-blur-sm rounded-2xl border border-white/50 overflow-hidden shadow-[0_4px_12px_rgba(0,0,0,0.02)] hover:shadow-[0_12px_24px_rgba(179,109,57,0.1)] hover:border-[#b36d39]/40 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between">
      ...
      {/* منطقة الصورة الفاتحة المريحة للعين */}
      <div className="bg-gradient-to-b from-[#fbf5f1] to-[#f5e4da]/40 h-44 flex items-center justify-center text-5xl border-b border-[#e6cebf]/30 relative group-hover:scale-105 transition-transform duration-300">
        🛍️
      </div>
      ...
      <div className="p-4 flex-1 flex flex-col justify-between">
        <div>
          ...
          <span className="text-[10px] bg-gradient-to-r from-[#d48c56]/20 to-[#b36d39]/20 text-[#8c4f22] px-2.5 py-1 rounded-md font-extrabold tracking-wide inline-block mb-4">
            مميز ★
          </span>
        </div>

        <div>
          {/* زر "أضف للسلة" التفاعلي ثلاثي الأبعاد باللون البرتقالي/الذهبي الفاخر */}
          <button
            onClick={() => addItem(product)}
            className="w-full bg-gradient-to-r from-[#d48c56] to-[#b36d39] hover:from-[#bd7a47] hover:to-[#9e5c2d] text-white text-xs font-bold py-2.5 rounded-xl shadow-[0_4px_10px_rgba(179,109,57,0.2)] hover:shadow-[0_6px_15px_rgba(179,109,57,0.35)] transition-all duration-300 active:scale-95"
          >
            أضف للسلة
          </button>
