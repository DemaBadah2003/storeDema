...
export default function DataTable({ columns, data, onDelete, onAdd, addButtonLabel }: DataTableProps) {
  return (
    <div className="bg-white p-6 rounded shadow border border-gray-100">
      {/* زر الإضافة - تم تغيير اللون إلى البني */}
      {onAdd && (
        <button 
          onClick={onAdd}
      ...
      <table className="w-full text-right border-collapse border border-gray-200">
        <thead>
          {/* رأس الجدول - ألوان هادئة ومتناسقة */}
        ...
        <tbody>
          {data.map((row) => (
            <tr key={row.id} className="border-b border-gray-200 hover:bg-orange-50 transition">
              {columns.map((col, idx) => (
                <td key={idx} className="py-4 px-4 border-x border-gray-200">{row[col.accessor]}</td>
              ))}
              <td className="py-4 px-4 border-x border-gray-200 flex gap-4">
                {/* أزرار الإجراءات - عند التحويم تظهر باللون البني */}
                <button className="text-gray-600 hover:text-[#5c4033] transition">✏️</button>
                <button onClick={() => onDelete(row.id)} className="text-gray-600 hover:text-red-700 transition">🗑️</button>
              </td>
            </tr>
          ))}
        </tbody>
